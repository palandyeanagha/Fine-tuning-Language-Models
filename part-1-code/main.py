...
# def create_augmented_dataloader(args, dataset):
#     """
#     Creates a dataloader with the original training data augmented with 5,000
#     randomly transformed examples using the custom_transform function.
#     """
#     from torch.utils.data import DataLoader
#     from datasets import Dataset, concatenate_datasets
#     # 1. Sample 5,000 examples from the training split
#     n_augmented = 5000
#     train_split = dataset['train']
#     sampled_examples = random.sample(list(train_split), n_augmented)

#     # 2. Apply custom_transform to each sampled example
#     transformed_examples = [custom_transform(ex) for ex in sampled_examples]

#     # 3. Combine transformed examples with the original training split
#     combined_examples = list(train_split) + transformed_examples

#     # 4. Convert to HuggingFace Dataset (for compatibility with tokenizer/DataLoader)
#     augmented_dataset = Dataset.from_list(combined_examples)

#     # 5. Tokenize the dataset using the same tokenizer in args
...
#     tokenized_dataset = augmented_dataset.map(tokenize_fn, batched=True)
...
def create_augmented_dataloader(args, dataset):
    ...
    # Sample a deterministic subset of training examples
    generator = random.Random(0)
    sampled_indices = generator.sample(range(len(train_split)), n_augmented)
    sampled_dataset = train_split.select(sampled_indices)

    # Apply transformation on the sampled subset only
    transformed_dataset = sampled_dataset.map(
        custom_transform, load_from_cache_file=False, desc="Applying augmentation transform"
    )

    # Concatenate original training data with transformed examples
    augmented_dataset = concatenate_datasets([train_split, transformed_dataset])
...
def create_transformed_dataloader(args, dataset, debug_transformation):
    ...
    transformed_dataset = dataset["test"].map(custom_transform, load_from_cache_file=False, desc="Applying custom transformation")
