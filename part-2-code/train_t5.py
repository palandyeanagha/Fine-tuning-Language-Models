...
def get_args():
    ...
    parser.add_argument('--learning_rate', type=float, default=3e-4,
                        help="Learning rate for fine-tuning (typically 3e-4 to 5e-4)")
...
def train(args, model, train_loader, dev_loader, optimizer, scheduler, start_epoch=0, best_f1=-1, epochs_since_improvement=0):
    ...
    print(f"Starting training from epoch {start_epoch}, best F1 so far: {best_f1:.4f}")
    ...
    for epoch in range(start_epoch, args.max_n_epochs):
        ...
        save_model(checkpoint_dir, model, best=False, optimizer=optimizer, scheduler=scheduler, epoch=epoch+1, best_f1=best_f1, epochs_since_improvement=epochs_since_improvement)
        if epochs_since_improvement == 0:
            save_model(checkpoint_dir, model, best=True, optimizer=optimizer, scheduler=scheduler, epoch=epoch+1, best_f1=best_f1, epochs_since_improvement=epochs_since_improvement)
...
def train_epoch(args, model, train_loader, optimizer, scheduler):
    ...
    for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(train_loader):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0) #version 2
        optimizer.step()
...
def eval_epoch(args, model, dev_loader, gt_sql_pth, model_sql_path, gt_record_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, decoder_input, decoder_targets, _ in tqdm(dev_loader, desc="Evaluating"):
            ...
            if non_pad.sum() > 0:
                loss = criterion(logits_flat[non_pad], targets_flat[non_pad])
            ...
            generation_config = GenerationConfig(
                max_new_tokens=512,
                num_beams= 8, #4,  # Beam search for better results
                do_sample=False,
                pad_token_id=tokenizer.pad_token_id,
                eos_token_id=tokenizer.eos_token_id,
                early_stopping=True,
                length_penalty=0.6,  # NEW
...
def test_inference(args, model, test_loader, model_sql_path, model_record_path):
    ...
    with torch.no_grad():
        for encoder_input, encoder_mask, initial_decoder_input, nl_queries in tqdm(test_loader, desc="Testing"):
            encoder_input = encoder_input.to(DEVICE)
            encoder_mask = encoder_mask.to(DEVICE)
...
def main():
    ...
    if args.resume:
        ...
        if os.path.exists(latest_checkpoint):
            ...
            model, start_epoch, best_f1, epochs_since_improvement = load_model_from_checkpoint(args, best=False, optimizer=optimizer, scheduler=scheduler)
            print(f"Resumed from epoch {start_epoch}, best F1: {best_f1:.4f}, epochs since improvement: {epochs_since_improvement}")
        else:
    ...
    # Train 
    train(args, model, train_loader, dev_loader, optimizer, scheduler, start_epoch=start_epoch, best_f1=best_f1, epochs_since_improvement=epochs_since_improvement)
